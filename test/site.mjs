...
test("installer assets are explicit and syntax-valid", () => {
  ...
  const lxcRuntimeManifest = readFileSync(`${root}/deploy/1helm-lxc-runtime-v2.conf`, "utf8");
  ...
  assert.match(lxcRuntimeManifest, /d5351325dc23e344c4974d7ff546e5e0c91b8e47a9caeb26f39cdc60eaad19e8/);
  assert.match(lxcHelper, /ASSET_ROOT="\/opt\/1helm\/runtime\/lxc\/images\/\$ONEHELM_LXC_IMAGE_SET"/, "image payloads live under their immutable manifest identity");
  assert.match(lxcHelper, /ACTIVE_CACHE_BASE="\$CACHE_BASE\/images\/\$ONEHELM_LXC_IMAGE_SET"[\s\S]*1helm-runtime-contract/, "the cache is namespaced and keyed by the complete helper, build, and digest contract");
  assert.match(lxcHelper, /ready\)[\s\S]*prepare_cache/, "host readiness verifies and repairs the exact installed image/cache contract");
  assert.match(lxcInstaller, /candidate=1[\s\S]*candidate_count=65535[\s\S]*namespace_covers_range/, "nested unprivileged hosts delegate their local non-root ID range");
  assert.match(lxcInstaller, /candidate_asset[\s\S]*install[\s\S]*mv -f -- "\$candidate_asset"/, "verified image payload replacement is atomic within its immutable image-set directory");

...
test("open chat threads present the persisted Board follow-up as a live countdown", () => {
  ...
  assert.match(state, /S\.threadFollowup = data\.followup \|\| null/, "thread snapshot hydration retains the persisted wake");
...
test("Scheduled Board cards cancel one wake without confirmation or agent invocation", () => {
  const board = readFileSync(new URL("src/client/board-operations.ts", root), "utf8");

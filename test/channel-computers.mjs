...
test("Apple channel-computer contract preserves isolation, files, wakes, archive, and exact ownership", async (t) => {
  ...
  db.run("UPDATE channel_computers SET last_update=?,last_update_attempt=0 WHERE channel_id=?", Date.now() - 8 * 24 * 60 * 60_000, alpha.channelId);
...
test("runtime digest and packaged image recipe stay pinned", async () => {
  ...
  assert.equal(computers.DEFAULT_CHANNEL_IMAGE, "local/1helm-channel-machine:1.1.16");

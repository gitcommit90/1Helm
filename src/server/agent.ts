...
const nativeEnv = (extra: Record<string, unknown> = {}): Record<string, string> => {
  ...
  // bootstrap command is typed into the live terminal or its history.
  const values: Record<string, unknown> = { ...process.env, ...extra, PATH: path, HELM_NATIVE_PATH: path };
  delete values.PROMPT;
  delete values.PS1;
  Object.assign(values, terminalPromptEnvironment(SHELL));
  ...
  const server = createServer(async (req, res) => {
    ...
    if (path === "/api/terminals" && req.method === "POST") {
      ...
      pty.onData((data) => {
        ...
        while (t.bytes > 256 * 1024 && t.scrollback.length > 1) t.bytes -= t.scrollback.shift()!.length;
      });
...
function attachTerm(ws: WebSocket, t: Term, apiKey: string): void {
  ...
  const forward = (): void => {
    if (onData) return;
    const startup = [...t.scrollback];
    onData = t.pty.onData((d) => ws.readyState === ws.OPEN && ws.send(d));
    for (const chunk of startup) if (ws.readyState === ws.OPEN) ws.send(chunk);
  };
  if (authed) forward();

...
type PhotonDispatch = (bot: Row, channelId: number, triggerId: number, threadRootId: number) => void | Promise<void>;
...
export function mapPhotonChannel(channelId: number, allowedUsers?: string[]): Record<string, unknown> {
  ...
  if (!channel || !resident?.id || resident.kind !== "channel") throw new Error("Choose an active resident channel.");
...
export async function deliverPhotonEvent(event: PhotonEvent): Promise<boolean> {
  ...
  if (bot && dispatchInbound) void dispatchInbound(bot, channelId, messageId, messageId);
  return true;
}
...
export async function startPhotonConnector(): Promise<void> {
  ...
  while (now() < deadline) {
    try { await sidecar("/health", { method: "POST", body: "{}" }); void consume(); return; } catch { await new Promise((resolve) => setTimeout(resolve, 250)); }
  }

...
export const HERMES_SKILL_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json";
const CATALOG_DIR = join(DATA_DIR, "skill-catalog");
const CATALOG_FILE = join(CATALOG_DIR, "hermes-index.json");
...
export function skillCatalogStatus(): CatalogStatus {
  const state = q1("SELECT * FROM skill_catalog_state WHERE source='hermes'");
  ...
  refreshPromise = (async () => {
    try {
      ...
      run(`INSERT INTO skill_catalog_state (source,url,generated_at,refreshed_at,skill_count,index_sha256,last_error)
        VALUES ('hermes',?,?,?,?,?,'') ON CONFLICT(source) DO UPDATE SET url=excluded.url,generated_at=excluded.generated_at,refreshed_at=excluded.refreshed_at,skill_count=excluded.skill_count,index_sha256=excluded.index_sha256,last_error=''`,
      HERMES_SKILL_INDEX_URL, index.generated_at, now(), index.skill_count, digest(bytes));
    } catch (error) {
      run(`INSERT INTO skill_catalog_state (source,url,generated_at,refreshed_at,skill_count,index_sha256,last_error)
        VALUES ('hermes',?,'',0,0,'',?) ON CONFLICT(source) DO UPDATE SET last_error=excluded.last_error`, HERMES_SKILL_INDEX_URL, safeText((error as Error).message, 1000));
...
export async function searchSkillCatalog(query: string, opts: { limit?: number; trust?: string } = {}): Promise<{ status: CatalogStatus; results: ExternalSkill[] }> {
  ...
  const trust = safeText(opts.trust, 40);
  const results = index.skills
    .filter((entry) => !trust || entry.trust_level === trust)
...
export async function inspectCatalogSkill(identifier: string): Promise<{ entry: ExternalSkill; installed: Row | null; installable: boolean; policy: string }> {
  ...
  const installable = ["builtin", "trusted"].includes(entry.trust_level) && Boolean(entry.repo && entry.path);
  ...
  return {
    ...
    installable,
    policy: installable
      ? "Curated GitHub-backed source metadata. 1Helm will independently resolve an immutable revision, bound and scan SKILL.md, hash it, and install it under the resident security boundary."
      : "Community content is discoverable but quarantined. It cannot auto-install until a stronger review path validates its source and contents.",
  };
}
...
async function resolveRevision(repo: string, path: string): Promise<string> {
  const headers: Record<string, string> = { accept: "application/vnd.github+json" };
  const token = String(process.env.GITHUB_TOKEN || process.env.GH_TOKEN || "");
  if (token) headers.authorization = `Bearer ${token}`;
  const url = `https://api.github.com/repos/${repo}/commits?path=${encodeURIComponent(`${path}/SKILL.md`)}&per_page=1`;
...
export async function installCatalogSkill(identifier: string, assignToAgentId?: number | null): Promise<Row> {
  ...
  const path = skillPath(inspection.entry.path);
  const revision = await resolveRevision(repo, path);
  const sourceUrl = `https://raw.githubusercontent.com/${repo}/${revision}/${path}/SKILL.md`;

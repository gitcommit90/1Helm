...
export async function execOnComputer(c: Computer, command: string, cwd?: string, waitSec = 60): Promise<{ status: string; exit_code: number | null; output: string }> {
  const res = await fetch(`${httpBase(c)}/execute?wait=${waitSec}`, {
    method: "POST", headers: authHeaders(c), body: JSON.stringify({ command, cwd }),
  });
  ...
  for (let i = 0; data.status === "running" && i < 4; i++) {
    const s = await fetch(`${httpBase(c)}/execute/${data.id}/status?wait=${waitSec}&offset=${offset}`, { headers: authHeaders(c) });
    if (!s.ok) break;
...
export async function createTerminal(c: Computer, cols: number, rows: number): Promise<string> {
  const res = await fetch(`${httpBase(c)}/api/terminals`, { method: "POST", headers: authHeaders(c), body: JSON.stringify({ cols, rows }) });

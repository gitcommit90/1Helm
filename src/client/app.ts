...
import { configureChannelSearchUi, openChannelSearch } from "./channel-search.ts";
import { configureAttachmentUi, renderMessageAttachments } from "./message-attachments.ts";
import { setAvatarUi } from "./avatar-ui.ts";
export { S } from "./state.ts";
configureWorkflowUi({ api, h, clear, icon, md, timeLabel });
configureChannelSearchUi({ api, h, clear, icon, md });
configureAttachmentUi({ h, icon, serverAssetUrl, getToken, stageCoworkPathLazy, navigateChannelView, openAuthenticatedFile, downloadAuthenticatedFile, appAlert });

...
import { createProviderModelRefresh } from "./provider-model-refresh.ts";

type RoutingView = "sources" | "routes" | "activity" | "quota" | "logs" | "endpoint";
type Dialog = (message: string) => Promise<boolean>;
const { modelAutoRefreshControls, openModelRefresh } = createProviderModelRefresh({ routingAction, add, clear, h, icon });
...
function accountCard(account: RoutingProvider, refresh: () => Promise<void>, confirm: Dialog, expandedAccounts: Set<string>): HTMLElement {
  ...
  add(details,
    ...
    account.mine === false ? null : modelAutoRefreshControls(account, refresh),

...
- **Confirmed visual cold deployment — 2026-07-10:** merged the visual identity overhaul in PR #2 (merge commit `d93fa30`) and redeployed `main` with `scripts/deploy-vps-fresh.sh`. The VPS workspace data was wiped before rebuild. The public endpoint at `http://167.233.229.141:8123` now reports `needs_setup: true`, `has_users: false`, `setup_complete: false`, and `provider_count: 0`. A public Chromium session rendered **Create the owner account** with title `1Helm`, the new `/brand/1helm.png` mark, no legacy CTRL PANE / Slack-clone copy, no purple numeral tile, and no console/page errors. `/`, `/app.css`, `/bundle.js`, and `/brand/1helm.png` returned HTTP 200; the mark returned `image/png`.

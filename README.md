...
- **Public autonomy contract** — `npm run benchmark:autonomy` emits deterministic machine-readable JSON verifying the shipped arsenal, narrow human-blocker gate, resident tool surface, wakeable recurring work, and audit-chain invariants for its executed fixture. It is explicitly not a live-model task-success or security score.
...
| `HELM_CHANNEL_MACHINE_IMAGE` | `local/1helm-channel-machine:1.1.19` | Versioned OCI machine image built from `container/Containerfile`. |

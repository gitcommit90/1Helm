...
const channelImage = JSON.parse(readFileSync(imageMetaPath, "utf8"));
if (channelImage.schema !== 1 || channelImage.kind !== "1helm-sealed-channel-image" || channelImage.architecture !== ociArchitecture
    || !/^[a-f0-9]{64}$/.test(String(channelImage.sha256 || "")) || digestFile(imageTar) !== channelImage.sha256
    || statSync(imageTar).size !== channelImage.bytes) {
  throw new Error("Sealed channel image bytes, architecture, or manifest do not match");
}

if (existsSync(output)) throw new Error(`Refusing to overwrite existing artifact: ${relative(root, output)}`);
...
try {
  ...
  const sourceArchive = run("git", ["archive", "--format=tar", "HEAD"], { encoding: "buffer", maxBuffer: 512 * 1024 * 1024 });
  ...
  writeFileSync(join(resourcesDir, "source-build.json"), `${JSON.stringify({ version, commit: headSha, source_archive_sha256: sourceArchiveSha256 }, null, 2)}\n`);
  mkdirSync(join(releaseRoot, "container"), { recursive: true });
  copyFileSync(imageTar, join(releaseRoot, "container", "channel-machine.oci.tar"));
  writeFileSync(join(releaseRoot, "container", "channel-machine.oci.sha256"), `${channelImage.sha256}\n`);
  copyFileSync(imageMetaPath, join(releaseRoot, "container", "channel-machine.oci.json"));
  ...
  console.log(`${basename(output)} ${statSync(output).size} bytes sha256=${onlineDigest}`);
